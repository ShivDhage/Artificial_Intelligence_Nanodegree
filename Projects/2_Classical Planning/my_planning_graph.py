...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        # Check if any effect of actionA negates any effect of actionB (or vice versa)
    ...
    def _interference(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        # Check if effects of actionA negate preconditions of actionB
        for effectA in self.children[actionA]:
            for precondB in self.parents[actionB]:
                if effectA == ~precondB:
                    return True
        # Check if effects of actionB negate preconditions of actionA
        for effectB in self.children[actionB]:
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # Check if any precondition of actionA is mutex with any precondition of actionB
        for precondA in self.parents[actionA]:
            for precondB in self.parents[actionB]:
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        layers.BaseLayer.parent_layer
        """
        # Get all actions that can produce literalA and literalB
        actionA_set = self.parents[literalA]
        actionB_set = self.parents[literalB]
        
        # If either literal has no supporting actions, they cannot both be achieved
        if not actionA_set or not actionB_set:
            return True
        
        # Check if all pairs of supporting actions are mutex
        for actionA in actionA_set:
            for actionB in actionB_set:
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        """
        # Keep track of level costs for each goal
        goal_levels = {}
        level_idx = 0
        
        # Incrementally expand the planning graph
        while not all(goal in goal_levels for goal in self.goal):
            # Check if all goals are in the current literal layer
            for goal in self.goal:
                if goal not in goal_levels and goal in self.literal_layers[level_idx]:
                    goal_levels[goal] = level_idx
            
            # If all goals are found, return their sum
            if all(goal in goal_levels for goal in self.goal):
                return sum(goal_levels.values())
            
            # If we've leveled and not all goals found, they're unreachable
            if self._is_leveled:
                break
        ...
        # Return sum of all goal levels found
        return sum(goal_levels.values())
    ...
    def h_maxlevel(self):
        ...
        """
        # Incrementally expand the planning graph until all goals are found
        level_idx = 0
        
        while True:
            # Check if all goals are in the current literal layer
            if all(goal in self.literal_layers[level_idx] for goal in self.goal):
                return level_idx
            
            # If we've leveled and goals still not all present, they're unreachable
            if self._is_leveled:
                return float('inf')
    ...
    def h_setlevel(self):
        ...
        """
        # Incrementally expand the planning graph until we find set level
        level_idx = 0
        
        while True:
            current_layer = self.literal_layers[level_idx]
            
            # Check if all goals are present in current layer
            if all(goal in current_layer for goal in self.goal):
                # Check if any pair of goals are mutex
                goals_are_mutex = False
                for goalA, goalB in combinations(self.goal, 2):
                    if current_layer.is_mutex(goalA, goalB):
                        goals_are_mutex = True
                        break
                
                # If no goals are mutex, we found the set level
                if not goals_are_mutex:
                    return level_idx
            
            # If we've leveled and haven't found set level, it's unreachable
            if self._is_leveled:
                return float('inf')
